...
const equationType = (
    ...
    // 💡 에디터가 현재 커서 위치를 뭐라고 부르는지 콘솔창에 띄워봅니다!
    if (currentNode) {
        console.log("현재 커서 위치의 문법 이름표:", currentNode.name);
    }

    while (currentNode) {
        const nodeName = currentNode.name;

        // 임시 방어벽: 이름표에 'math'나 'equation'이나 '$' 기호가 들어가면 수식으로 인정!
        if (nodeName.toLowerCase().includes("math") || nodeName.toLowerCase().includes("equation") || nodeName === "$") {
            return {
                type: MathMode.DollarInline,
                inner_bounds: { start: currentNode.from, end: currentNode.to },
                outer_bounds: { start: currentNode.from, end: currentNode.to },
            };
        }
